...
class SAM6DPEM_ROS:
    def __init__(self, config_file, ism_results_dir="/tmp/sam6d_ism"):
        ...
        for obj_name in self.object_templates.keys():
            topic_name = f'/pose_estimator/sam6dpem/{obj_name}/pose'
    ...
    def estimate_pose(self, req):
        ...
        for result in sorted_best_results:
            try:
                ...
                # Rotate -180 degrees around X, Y, and Z axes
                ...
                if class_name in self.pose_publishers:
                    pose_stamped = PoseStamped()
                    pose_stamped.header.stamp = rospy.Time.now()
                    pose_stamped.header.frame_id = "head_rgbd_sensor_rgb_frame"  # Adjust frame_id as needed
                    pose_stamped.pose = pose_msg
                    self.pose_publishers[class_name].publish(pose_stamped)
                    print(f"     -> Published pose to /pose_estimator/sam6dpem/{class_name}/pose")
